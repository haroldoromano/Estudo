...
local function CastSpell(idxCaster)
	...
	for y = 1,8 do
		for x = 1,8 do
			if (GET_GEM(x,y) == 5 or GET_GEM(x,y) == 15) then
				DELETE_GEM(x,y);
	...
	Std_CastSpellEffect(idxCaster,SPELLFX_HEALING,GET_TEXT("[SPELL_SCHM_NAME]"));
end
...
local function ShouldAICastSpell(idxCaster)
	...
	for y = 1,8 do
		for x = 1,8 do
			if (GET_GEM(x,y) == 5 or GET_GEM(x,y) == 15) then
				numSkulls = numSkulls+1;
			end
		end
	end
	chance = chance + numSkulls;
