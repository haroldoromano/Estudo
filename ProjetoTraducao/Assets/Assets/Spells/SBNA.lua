...
local function CastSpell(idxCaster)
	...
	ADD_TEMP_SKILL(idxCaster,SKILL_AIR,numGems );
	local msg = GET_TEXT_FORMAT_INT("[SPELL_EFFECT_ADDAIR]",numGems );
	ADD_TEXT_MESSAGE_TO_CHARACTER(idxCaster, msg,2,MESSAGE_YELLOW);

	-- Special Effect
	Std_CastSpellEffect(idxCaster,SPELLFX_SYMBOL,GET_TEXT("[SPELL_SBNA_NAME]"));
