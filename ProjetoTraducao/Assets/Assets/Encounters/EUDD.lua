...
local function OnQueryAppearance()
	if (QUEST_HERO_LEVEL() >= myLevel and 
	    PERCENTILE_CHANCE() <= 2 and
		QUEST_IS_VISIBLE("CLBC") == 1 and
		QUEST_IS_VISIBLE("WSOI") == 1) then
			return 1;
	end
	return 0;
end
...
local function OnQueryDisappearance()
	if(QUEST_IS_VISIBLE("CLBC") == 0 and
		QUEST_IS_VISIBLE("WSOI") == 0) then
		return 1;
	elseif (PERCENTILE_CHANCE() <= 1) then
			return 1;
	end
	return 0;
end
