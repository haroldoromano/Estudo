...
local function OnEnd()
	questState = 0; -- repeatable
	
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q0R1_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q0R1_REWARD]");
	QUEST_REWARD_MENU(myTitle,myMessage);
	QUEST_REWARD_GOLD(125);
	QUEST_REWARD_XP(50);
end
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q0R1_ACTION]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
			...
			myMessage = GET_TEXT("[QUEST_Q0R1_FAILURE]");
