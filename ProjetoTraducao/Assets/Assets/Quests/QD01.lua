...
local function OnBegin()
	questState = 1;
	QUEST_CONVERSATION("Assets\\Conversations\\Conv_QD01a.xml",myOrigin);
end
...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_QD01_NAME]");
	local myMessage = GET_TEXT("[QUEST_QD01_REWARD]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_QD01_ACTION]");
	end
	return "";
end
...
local function OnExecuteAction(idLocation)
	if (questState==1) then
		if(QUEST_HERO_HAS_COMPANION_SELECTED("NDRO")==1) then
			...
			local myTitle = GET_TEXT("[QUEST_QD01_NAME]");
			local myMessage = GET_TEXT("[QUEST_QD01_DRONG]");
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		local myMessage;
		if (success == 1) then
			QUEST_CONVERSATION("Assets\\Conversations\\Conv_QD01b.xml",myLocation);
		else
			local myTitle = GET_TEXT("[QUEST_QD01_NAME]");
			local myMessage = GET_TEXT("[QUEST_QD01_FAILURE]");
			QUEST_MESSAGE(myTitle,myMessage,myLocation);
			QUEST_UPDATE_MAP();
		end
	end
...
local function OnQueryProgress()
	if (questState == 1) then
		return GET_TEXT("[QUEST_QD01_STEP1]");
	end
	return "";
end
