...
local function OnBegin()
	questState = 1;
	QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q3D0a.xml",myOrigin);
end
...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q3D0_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q3D0_REWARD]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q3D0_ACTION]");
	elseif (idLocation==myOrigin and questState==2) then
			return GET_TEXT("[QUEST_Q3D0_RETURN]");
	end
	return "";
end
...
local function OnExecuteAction(idLocation)
	if (questState==1) then
		local myTitle = GET_TEXT("[QUEST_Q3D0_NAME]");
		local myMessage = GET_TEXT("[QUEST_Q3D0_AMBUSH]");
		QUEST_MESSAGE_CALLBACK(myTitle,myMessage,idLocation,myCode,"CallbackMsg");
	elseif (questState==2) then
		QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q3D0b.xml",idLocation);
	end
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
			...
			myMessage = GET_TEXT("[QUEST_Q3D0_KILL]");
		else
			myMessage = GET_TEXT("[QUEST_Q3D0_FAILURE]");
		end
...
local function OnQueryProgress()
	if (questState == 1) then
		return GET_TEXT("[QUEST_Q3D0_STEP1]");
	elseif (questState == 2) then
		return GET_TEXT("[QUEST_Q3D0_STEP2]");
	end
	return "";
end
