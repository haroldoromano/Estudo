...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q2I0_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q2I0_REWARD]");
	QUEST_REWARD_MENU(myTitle,myMessage);
	QUEST_REWARD_XP(250);
	QUEST_REWARD_ITEM("IDHE");
end
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q2I0_ACTION]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
			...
			myMessage = GET_TEXT("[QUEST_Q2I0_FAILURE]");
