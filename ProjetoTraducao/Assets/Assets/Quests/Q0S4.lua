...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q0S4_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q0S4_REWARD]");
...
local function OnEnterLocation(idLocation)
	if (idLocation == myDestination and questState == 1) then
		local myTitle = GET_TEXT("[QUEST_Q0S4_NAME]");
		local myMessage = GET_TEXT("[QUEST_Q0S4_MESSAGE]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q0S4_ACTION]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
			...
			myMessage = GET_TEXT("[QUEST_Q0S4_FAILURE]");
