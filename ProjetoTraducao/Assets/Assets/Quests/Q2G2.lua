...
local function OnBegin()
	questState = 1;
	QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q2G2a.xml",myOrigin);
end
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q2G2_ACTION]");
	elseif (idLocation==myOrigin and questState==2) then
			return GET_TEXT("[QUEST_Q2G2_RETURN]");
	end
	return "";
end
...
local function OnExecuteAction(idLocation)
	...
	elseif (questState==2) then
		QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q2G2b.xml",idLocation);
	end
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		local myTitle = GET_TEXT("[QUEST_Q2G2_NAME]");
		local myMessage;
		if (success == 1) then
	  		questState = 2;
			myMessage = GET_TEXT("[QUEST_Q2G2_KILL]");
		else
			myMessage = GET_TEXT("[QUEST_Q2G2_FAILURE]");
		end
...
local function OnQueryProgress()
	if (questState == 1) then
		return GET_TEXT("[QUEST_Q2G2_STEP1]");
	elseif (questState == 2) then
		return GET_TEXT("[QUEST_Q2G2_STEP2]");
	end
	return "";
end
