...
local function OnBegin()
	questState = 1;
	QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q1D1a.xml",myOrigin);
end
...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q1D1_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q1D1_REWARD]");
	QUEST_REWARD_MENU(myTitle,myMessage);
	QUEST_REWARD_ITEM("ITBS");
	QUEST_REWARD_XP(50);
end
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q1D1_ACTION]");
	elseif (idLocation==myEnd and questState==2) then
			return GET_TEXT("[QUEST_Q1D1_RETURN]");
	end
	return "";
end
...
local function OnExecuteAction(idLocation)
	...
	elseif (questState==2) then
		QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q1D1b.xml",idLocation);
	end
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
	  		questState = 2;
			QUEST_SET_RUIN_DONE(myLocation);
			myMessage = GET_TEXT("[QUEST_Q1D1_KILL]");
		else
			myMessage = GET_TEXT("[QUEST_Q1D1_FAILURE]");
		end
...
local function OnQueryProgress()
	if (questState == 1) then
		return GET_TEXT("[QUEST_Q1D1_STEP1]");
	elseif (questState == 2) then
		return GET_TEXT("[QUEST_Q1D1_STEP2]");
	end
	return "";
end
