...
--
local myCode     = "Q3A2";
local myLocation = "RHAG";
local myOrigin   = "CEHL";
local myWaypoint = "WRAG";
...
local function OnBegin()
	...
	Std_AddEncounter(myOrigin,myWaypoint ,"ENHA");
	QUEST_UPDATE_MAP();
end
...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q3A2_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q3A2_REWARD]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q3A2_ACTION]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
			...
			myMessage = GET_TEXT("[QUEST_Q3A2_FAILURE]");
