...
local function OnBegin()
	questState = 1;
	QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q0Q3a.xml",myOrigin);
	QUEST_SET_VISIBILITY("WSKE",1);
	QUEST_SET_VISIBILITY("CDRA",1);
...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q0Q3_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q0Q3_REWARD]");
...
local function OnEnterLocation(idLocation)
	if (idLocation == myLocation and questState == 1) then
		local myTitle = GET_TEXT("[QUEST_Q0Q3_NAME]");
		local myMessage = GET_TEXT("[QUEST_Q0Q3_MESSAGE]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q0Q3_ACTION]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
			QUEST_SET_RUIN_DONE(myLocation);
	  		QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q0Q3b.xml",idLocation);
		else
			myMessage = GET_TEXT("[QUEST_Q0Q3_FAILURE]");
