...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q1E1_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q1E1_REWARD]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q1E1_ACTION]");
	elseif (idLocation==myOrigin and questState==2) then
			return GET_TEXT("[QUEST_Q1E1_RETURN]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
	  		questState = 2;
			myMessage = GET_TEXT("[QUEST_Q1E1_KILL]");
		else
			myMessage = GET_TEXT("[QUEST_Q1E1_FAILURE]");
		end
...
local function OnQueryProgress()
	if (questState == 1) then
		return GET_TEXT("[QUEST_Q1E1_STEP1]");
	elseif (questState == 2) then
		return GET_TEXT("[QUEST_Q1E1_STEP2]");
	end
	return "";
end
