...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q0S1_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q0S1_REWARD]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
		return GET_TEXT("[QUEST_Q0S1_ACTION1]");
	elseif (idLocation==myLocation and questState==2) then
		return GET_TEXT("[QUEST_Q0S1_ACTION2]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		local myMessage;
		if (success == 1) then
			myMessage = GET_TEXT("[QUEST_Q0S1_SUCCESS1]");
			questState = 2;
		else
			myMessage = GET_TEXT("[QUEST_Q0S1_FAILURE1]");
		end
	...
	elseif (questState==2) then
		...
		if (success == 1) then
			...
			myMessage = GET_TEXT("[QUEST_Q0S1_FAILURE2]");
...
local function OnQueryProgress()
	if (questState==1) then
		return GET_TEXT("[QUEST_Q0S1_PROGRESS1]");
	elseif (questState==2) then
		return GET_TEXT("[QUEST_Q0S1_PROGRESS2]");
	end
	return "";
end
