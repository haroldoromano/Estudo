...
local function OnBegin()
	questState = 1;
	QUEST_CONVERSATION("Assets\\Conversations\\Conv_QK02a.xml",myOrigin);
end
...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_QK02_NAME]");
	local myMessage = GET_TEXT("[QUEST_QK02_REWARD]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_QK02_ACTION]");
	end
	return "";
end
...
local function OnExecuteAction(idLocation)
	if (questState==1) then
		if (QUEST_HERO_HAS_COMPANION_SELECTED("NKHA")==0) then	
			local myTitle = GET_TEXT("[QUEST_QK02_NAME]");
			local myMessage = GET_TEXT("[QUEST_QK02_NOCOMP]");
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
			QUEST_SET_RUIN_DONE(myLocation);
			QUEST_CONVERSATION("Assets\\Conversations\\Conv_QK02b.xml",myLocation);
		else
			local myTitle = GET_TEXT("[QUEST_QK02_NAME]");
			local myMessage = GET_TEXT("[QUEST_QK02_FAILURE]");
...
local function OnQueryProgress()
	if (questState == 1) then
		return GET_TEXT("[QUEST_QK02_STEP1]");
	end
	return "";
end
