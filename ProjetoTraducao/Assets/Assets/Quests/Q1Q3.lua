...
local function OnBegin()
	questState = 1;
	QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q1Q3a.xml",myOrigin);
end
...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q1Q3_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q1Q3_REWARD]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q1Q3_ACTION]");
	elseif (idLocation==myOrigin and questState==2) then
			return GET_TEXT("[QUEST_Q1Q3_RETURN]");
	end
	return "";
end
...
local function OnExecuteAction(idLocation)
	...
	elseif (questState==2) then
		QUEST_CONVERSATION("Assets\\Conversations\\Conv_Q1Q3b.xml",myOrigin);
	end
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
	  		questState = 2;
			myMessage = GET_TEXT("[QUEST_Q1Q3_KILL]");
		else
			lostToMonster = 1;
			myMessage = GET_TEXT("[QUEST_Q1Q3_FAILURE]");
		end
...
local function OnQueryProgress()
	if (questState == 1) then
		return GET_TEXT("[QUEST_Q1Q3_STEP1]");
	elseif (questState == 2) then
		return GET_TEXT("[QUEST_Q1Q3_STEP2]");
	end
	return "";
end
