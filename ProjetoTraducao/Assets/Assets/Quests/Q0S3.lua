...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q0S3_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q0S3_REWARD]");
	QUEST_REWARD_MENU(myTitle,myMessage);
	QUEST_REWARD_XP(50);
	QUEST_REWARD_ITEM("IEBW");
end
...
local function OnEnterLocation(idLocation)
	if ( questState == 1 and idLocation==myLocation ) then
		local myTitle = GET_TEXT("[QUEST_Q0S3_NAME]");
		local myMessage = GET_TEXT("[QUEST_Q0S3_ARRIVE]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q0S3_ACTION]");
	elseif (idLocation==myOrigin and questState==2) then
			return GET_TEXT("[QUEST_Q0S3_RETURN]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
	  		questState = 2;
			QUEST_SET_RUIN_DONE(myLocation);
			myMessage = GET_TEXT("[QUEST_Q0S3_KILL]");
		else
			lostToMonster = 1;
			myMessage = GET_TEXT("[QUEST_Q0S3_FAILURE]");
		end
		QUEST_MESSAGE(myTitle,myMessage,myLocation);
...
local function OnQueryProgress()
	if (questState == 1) then
		return GET_TEXT("[QUEST_Q0S3_STEP1]");
	elseif (questState == 2) then
		return GET_TEXT("[QUEST_Q0S3_STEP2]");
	end
	return "";
end
