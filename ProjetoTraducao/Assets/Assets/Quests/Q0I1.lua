...
local function OnEnd()
	...
	-- Reward
	local myTitle = GET_TEXT("[QUEST_Q0I1_NAME]");
	local myMessage = GET_TEXT("[QUEST_Q0I1_REWARD]");
...
local function OnQueryAction(idLocation)
	if (idLocation==myLocation and questState==1) then
			return GET_TEXT("[QUEST_Q0I1_ACTION]");
	end
	return "";
end
...
local function OnCompleteAction(idLocation, success)
	...
	if (questState==1) then
		...
		if (success == 1) then
			...
			myMessage = GET_TEXT("[QUEST_Q0I1_FAILURE]");
